...
#define ENGINE_STEP_DELAY 100 //Delay per ogni step di motore
...
int engines_movement_controlled(float duration, int power_left, int power_right) {
   ...
   logDebug(String("Starting wheel engine [controlled] | Duration (s): ")+String(duration) + " | Left wheel power: " + String(power_left) + String(" | Right wheel power: ") + String(power_right));
...
int engines_movement(float duration, int power_left, int power_right) {
  ...
  logDebug(String("Starting wheel engine [not-controlled] | Duration (s): ")+String(duration) + " | Left wheel power: " + String(power_left) + String(" | Right wheel power: ") + String(power_right));
  ...
  {
    logDebug(String("Detected error in movement result - Sopping engines"));
  }
return movement_result;
}
